...
impl MSize {
    ...
    pub fn align_up_to_page(&self) -> Self {
        MSize((self.0 + PAGE_SIZE - 1) & !(PAGE_SIZE - 1))
    }
}
...
unsafe fn get_pml4_from_cr3() -> &'static mut PageTableNode {
    let cr3 = read_cr3();
    unsafe { &mut *(cr3 as *mut PageTableNode) }
}

pub unsafe fn get_page_table_from_cr3() -> PageTable {
    let pml4 = unsafe { get_pml4_from_cr3() };
    unsafe {
        PageTable {
            pml4: core::ptr::read(pml4),
        }
    }
}

pub fn init_early_paging(kernel_start_phys: PhysAddr, kernel_start_virt: VirtAddr, size: MSize) {
